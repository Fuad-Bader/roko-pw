...
async function decryptData(key, ivB64, ciphertextB64) {
...
async function wrapKey(keyToWrap, wrappingKey) {
  ...
  return { iv: ab2b64(iv.buffer), wrappedKey: ab2b64(wrapped) };
}
async function unwrapKey(wrappedKeyB64, ivB64, wrappingKey) {
...
async function listServerVaults(remote) {
...
async function createServerVault(remote, name) {
  const res = await fetch(`${remote.serverUrl}/api/vaults`, {
    method: "POST",
...
async function inviteToVault(remote, vaultId, email, vaultPassword) {
...
async function loadVault(_backend, _id, _remote) {
  return window.electronAPI.loadVault();
}
async function saveVault(_backend, _id, vault, _remote) {
  return window.electronAPI.saveVault(vault);
}
async function exportVaultToFile(vault) {
...
function normaliseMnemonic(raw) {
  return raw.trim().split(/\s+/).map((w) => w.toLowerCase().trim()).filter(Boolean);
}
const VaultContext = reactExports.createContext(null);
function useVault() {
  const ctx = reactExports.useContext(VaultContext);
...
function readSettings() {
  ...
  const fresh = { backend: "local", vaultId: crypto.randomUUID(), serverUrl: "" };
...
function VaultProvider({ children }) {
  ...
  const [hasRecovery, setHasRecovery] = reactExports.useState(false);
  const [needsNewPassword, setNeedsNewPassword] = reactExports.useState(false);
  const [serverSession, setServerSession] = reactExports.useState(null);
  ...
  const recoverWithPhrase = reactExports.useCallback(async (words) => {
    ...
    try {
      ...
      setError(
        err instanceof Error && err.message.includes("no recovery") ? err.message : "Incorrect recovery phrase. Check each word and try again."
      );
    }
  }, []);
  ...
  const verifyOtp = reactExports.useCallback(async (serverUrl, email, otp) => {
    ...
    try {
      ...
      const session = { serverUrl, token, userId: me.id, email: me.email, expiresAt };
  ...
  const logoutServer = reactExports.useCallback(() => {
    writeSession(null);
  ...
  const refreshServerVaults = reactExports.useCallback(async () => {
    const session = readSession();
    if (!session) return;
    setServerLoading(true);
    try {
  ...
  const handleCreateServerVault = reactExports.useCallback(async (name) => {
    const session = readSession();
...
function RecoverVault({ onCancel }) {
  ...
  const [inputs, setInputs] = reactExports.useState(Array(WORD_COUNT).fill(""));
...
function ServerLogin({ onVaultSelected, onCancel }) {
  ...
  const [step, setStep] = reactExports.useState(serverSession ? "vaults" : "url");
